...
Vue 3 выбран за оптимальный баланс простоты, производительности и наличия enterprise-ready UI-библиотеки PrimeVue.
...
**PrimeVue 3.50 — библиотека UI-компонентов.**

23 компонента PrimeVue, используемых в проекте:

| Компонент | Назначение | Используется в |
|-----------|-----------|---------------|
| DataTable | Таблицы с сортировкой/пагинацией | Admin.vue |
| Column | Столбцы DataTable | Admin.vue |
| Dialog | Модальные окна | QuestionDetailDialog.vue |
| Button | Кнопки (primary, secondary, danger) | Все views |
| InputText | Текстовые поля | Формы |
| Textarea | Многострочные поля | Форма ответа |
| Dropdown | Выпадающие списки | Фильтры |
| Tag | Метки (topic, difficulty) | QuestionCard.vue |
| ProgressBar | Прогресс-бар pipeline | Admin.vue |
| Toast | Уведомления | Все views |
| ConfirmDialog | Подтверждение действий | Admin.vue |
| TabView | Вкладки | Admin.vue |
| TabPanel | Панели вкладок | Admin.vue |
| Badge | Числовые метки | NavBar.vue |
| Chip | Теги/навыки | QuestionDetail.vue |
| Skeleton | Загрузочные заглушки | Списки |
| Card | Карточки | Home.vue |
| Tooltip | Подсказки | Кнопки |
| Paginator | Пагинация | Списки |
| Slider | Ползунки | Trainer.vue |
| Accordion | Аккордеоны | Admin logs |
| AccordionTab | Секции аккордеона | Admin logs |
| ProgressSpinner | Спиннер загрузки | Все views |

Тема: `lara-dark-purple` — тёмная с фиолетовыми акцентами.

---

**Pinia 2.1.7 — управление состоянием.**

Два хранилища (stores) управляют глобальным состоянием:
...
3 конфигурации для разных сред:
...
2. **Компонент TechSelector** (308 строк) — выбор профессии/технологии:
   - Режим переключения: «По технологиям» / «По профессиям» (PrimeVue SelectButton);
   - Сетка карточек с CSS Grid (`auto-fill`, `minmax(180px, 1fr)`);
   - 26 профессий с уникальными градиентами;
   - Каждая карточка показывает количество вопросов (`getProfessions()` API).

Пример данных профессий:

| Профессия | Градиент | Иконка |
|-----------|---------|--------|
| Frontend | `#f093fb → #f5576c` | pi-palette |
| Backend | `#667eea → #764ba2` | pi-server |
| Python | `#4facfe → #00f2fe` | pi-code |
| Java | `#fa709a → #fee140` | pi-code |
| DevOps | `#fcb69f → #ffecd2` | pi-cloud |
| QA | `#a8edea → #fed6e3` | pi-check-circle |
| Data Science | `#fbc2eb → #a6c1ee` | pi-chart-bar |

3. **Feature-карточки** — 6 блоков с описанием возможностей:
   - Тренажёр SM-2, Записи собеседований, Вопросы из интервью, Тестовые задания, Навыки из вакансий, Предложить видео.


### 4.3.2. Тренажёр SM-2 (Trainer.vue, 601 строка)

Самый сложный view в проекте. Реализует полный цикл интервального повторения:

**Режим выбора (setup):**
- Dropdown для выбора темы (из API `GET /api/questions → unique topics`);
- Dropdown для выбора сложности (junior / middle / senior);
- Slider для количества карточек (5–50, шаг 1);
- Checkbox «Приоритизировать повтор» (сначала показывать просроченные).

**Статистическая панель:**
- Новые (new): карточки с `repetitions = 0`;
- К повторению (review): карточки с `next_review < NOW()`;
- Выученные (learned): карточки с `next_review > NOW()`.

**Режим карточек (flashcard):**
- Передняя сторона: текст вопроса;
- Обратная сторона: текст ответа (по нажатию «Показать ответ»);
- Кнопки оценки: «На повтор» (q < 3), «Знаю» (q ≥ 3);
- ProgressBar: текущая позиция / всего карточек.
...
### 4.3.3. Административная панель (Admin.vue, 873 строки)

Централизованная панель управления с PrimeVue TabView (4 вкладки):

**Вкладка 1: Вопросы (QuestionApproval)**
- DataTable со столбцами: ID, текст, тема, сложность, вероятность, статус;
- Действия: одобрить (POST approve), отклонить (POST revoke), редактировать (PUT), удалить (DELETE);
...
**Вкладка 2: Видео (VideoManager)**
- DataTable: название, платформа, количество вопросов, дата обработки;
- Действия: просмотр вопросов видео, переименование, удаление;
- Платформо-специфичные иконки (🎬 YouTube, 🔵 VK, 🎥 Rutube);

**Вкладка 3: Предложения (SuggestionsManager)**
- Таблица предложений от пользователей;
- Статусы: pending → approved / rejected / processing → completed;
- Администратор оставляет комментарий и меняет статус.

**Вкладка 4: Обратная связь (FeedbackManager)**
- Таблица фидбэков с фильтром resolved/unresolved;
- Типы: like, dislike, report, suggestion, answer_quality;
- Администратор помечает как resolved.

**Панель задач (верхняя часть):**
- Активные задачи с прогресс-барами;
- Expandable лог: каждая строка = этап pipeline с таймстампом;
- Кнопка «Загрузить видео» → Dialog с полями URL, тема, уровень;
- Кнопка «Загрузить файл» → multipart upload (VideoUpload).
...
| Секция | Данные | Компонент |
|--------|--------|-----------|
| Заголовок | Текст вопроса, тема (Tag), сложность (Tag), вероятность | — |
| Ответ | Автогенерированный ответ (LLM) | Markdown-форматирование |
...
| UGC-ответы | Пользовательские ответы с голосованием | vote up/down |
| Действия | Закладка, заметка, обратная связь | Button, Dialog |
...
2. **Реализована клиентская часть** на Vue 3 — 9 представлений, 12 компонентов, 23 PrimeVue-компонента. Централизованный API-клиент (334 строки, 67 методов), Pinia store с polling.
...
8. **Реализована контейнеризация** — 3 конфигурации Docker Compose (CPU/CPU-opt/GPU), auto-migration, мониторинг через `/health`. Суммарный размер образов: 6.9 ГБ.
